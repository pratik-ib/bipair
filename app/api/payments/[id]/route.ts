...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const response = NextResponse.json({ success: true, data: { id: data.id, status: data.status, amount: data.amount, currency: data.currency, transactionRef: data.transaction_ref, createdAt: data.created_at } });
